...
def get_args():
    ...
    parser.add_argument("--vol_dir", type=str, default=None,
                        help="If provided, the volume "
                        "recording will be written to this dir")
...
def run(args):
    ...
    for line in sys.stdin.readlines():
        ...
        if line.strip()[-1] == '|':
            ...
            outwav = args.vol_dir+'/'+' '.join(parts[1:]).split('/')[-1].split('.')[0]+'_vol.wav'
            print ('{id} sox --vol {vol} -t wav {wav} -t wav {owav}'.format(id=parts[0],
                wav=' '.join(parts[1:]), vol=vol, owav=outwav))
