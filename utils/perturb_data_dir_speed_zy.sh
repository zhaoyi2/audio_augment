...
outdir=$4
label="sp"
spk_suffix="_"$label$factor
utt_suffix="_"$label$factor
...
cat $srcdir/utt2spk | awk -v p=$utt_suffix '{printf("%s %s%s\n", $1, $1, p);}' > $destdir/utt_map
cat $srcdir/spk2utt | awk -v p=$spk_suffix '{printf("%s %s%s\n", $1, $1, p);}' > $destdir/spk_map
if [ ! -f $srcdir/utt2uniq ]; then
  cat $srcdir/utt2spk | awk -v p=$utt_suffix '{printf("%s%s %s\n", $1, p, $1);}' > $destdir/utt2uniq
else
  cat $srcdir/utt2uniq | awk -v p=$utt_suffix '{printf("%s%s %s\n", $1, p, $2);}' > $destdir/utt2uniq
fi
...
if [ -f $srcdir/segments ]; then
  ...
  cat $srcdir/wav.scp | awk -v p=$spk_suffix '{printf("%s %s%s\n", $1, $1, p);}' > $destdir/reco_map
  ...
  if [ -f $srcdir/wav.scp ]; then
    utils/apply_map.pl -f 1 $destdir/utt_map <$srcdir/wav.scp | sed 's/| *$/ |/' | \
     # Handle three cases of rxfilenames appropriately; "input piped command", "file offset" and "filename" 
     awk -v factor=$factor -v outdir=$outdir \
       '{wid=$1; $1=""; if ($NF=="|") {print wid $_ " sox -t wav - -t wav - speed " factor " |"}
         else if (match($0, /:[0-9]+$/)) {print wid " wav-copy" $_ " - | sox -t wav - -t wav - speed " factor " |"} 
         else {print wid " sox -t wav" $_ " -t wav "  outdir"/" wid ".wav speed " factor}}' > $destdir/wav.scp
  fi
fi
