...
from reverberate_data_dir_zy import ParseFileToDict
from reverberate_data_dir_zy import WriteDictToFile
...
def GetArgs():
    ...
    parser.add_argument('--out_dir', type=str, dest = "out_dir", default = '', help='record save dir.')
...
def AugmentWav(utt, wav, dur, fg_snr_opts, bg_snr_opts, fg_noise_utts, \
    bg_noise_utts, noise_wavs, noise2dur, interval, num_opts,out_dir):
    ...
    if wav.strip()[-1] != "|":
        new_wav = "wav-reverberate --shift-output=true " + noises_str + " " \
            + start_times_str + " " + snrs_str + " " + wav +' '+ out_dir+'/'+wav.split('/')[-1].split('.')[0]+'_'+out_dir.split('_')[-1]+'.wav'
