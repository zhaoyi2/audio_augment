...
def GetArgs():
    ...
    parser.add_argument("--reverberation-wav-dir", type=str, dest = "rvb_dir", default=None,
                        help="Directory of reverberation waves."
                        "E.g. --reverberation-wav-dir /home/data/rvb")
    ...
    parser.add_argument('--foreground-snrs-lower-bound', type=float, dest = "foreground_snr_lower", default = 10,
                        help="Foreground SNRs lower bound."
                        "The foreground SNRs will be randomly defined between the foreground-snrs-lower-bound and foreground-snrs-upper-bound.")
    parser.add_argument('--foreground-snrs-upper-bound', type=float, dest = "foreground_snr_upper", default = 20,
                        help="Foreground SNRs upper bound."
                        "The foreground SNRs will be randomly defined between the foreground-snrs-lower-bound and foreground-snrs-upper-bound.")
    parser.add_argument('--background-snrs-lower-bound', type=float, dest = "background_snr_lower", default = 10, 
                        help="Background SNRs lower bound."
                        "The background SNRs will be randomly defined between the background_snrs-lower-bound and background_snrs_upper-bound.")
    parser.add_argument('--background-snrs-upper-bound', type=float, dest = "background_snr_upper", default = 20, 
                        help="Background SNRs upper bound."
                        "The background SNRs will be randomly defined between the background_snrs-lower-bound and background_snrs_upper-bound.")
    ...
    parser.add_argument("--max_noises_adding", type=int, default = 1,
    ...
    parser.add_argument("--normalize-output", type=str, choices=['true', 'false'], default = 'true', dest= "normalize_output",
                        help="If true, then after reverberating and possibly adding noise, scale so that the signal energy is the same as the original input signal.")
    parser.add_argument("--verbose", type=int, default=0, dest = "verbose", help="Verbose level (higher->more logging)")
...
def CheckArgs(args):
    ...
    if args.max_noises_adding < 0:
        raise Exception("--max-noises-adding cannot be negative")
...
def AddPointSourceNoise(noise_addition_descriptor,  # descriptor to store the information of the noise added
                        ...
                        foreground_snr_lower, # the SNR for adding the foreground noises lower bound
                        foreground_snr_upper, # the SNR for adding the foreground noises upper bound
                        background_snr_lower, # the SNR for adding the background noises lower bound
                        background_snr_upper, # the SNR for adding the background noises upper bound
    ...
    if len(pointsource_noise_list) > 0 and random.random() < pointsource_noise_addition_probability and max_noises_recording >= 1:
        for k in range(random.randint(1, max_noises_recording)):
            ...
            if noise.bg_fg_type == "background":
                ...
                background_snrs = random.uniform(background_snr_lower, background_snr_upper)
                noise_addition_descriptor['snrs'].append(background_snrs)
            else:
                ...
                foreground_snrs = random.uniform(foreground_snr_lower, foreground_snr_upper)
                noise_addition_descriptor['snrs'].append(foreground_snrs)
...
def GenerateReverberatedWavScp(wav_scp,  # a dictionary whose values are the Kaldi-IO strings of the speech recordings
                               ...
                               isotropic_noise_addition_probability, # Probability of adding isotropic noises
                               pointsource_noise_addition_probability, # Probability of adding point-source noises
                               max_noises_adding, # maximum number of point-source noises that can be added to a recording
                               rvb_dir, # reverberation waves direcory
                               normalize_output, # scale so that the signal energy is the same as the original input signal. 
    ...
    for i in range(start_index, num_replicas+1):
        for recording_id in keys:
            ...
            if recording_id not in durations.keys():
                wav_error.write((recording_id+'\n').encode())
                continue
            speech_dur = durations[recording_id]
            max_noises_recording = max_noises_adding
            ...
            if reverberate_opts == "" or i == 0:
                ...
                wav_corrupted_pipe = "{0} wav-reverberate --normalize-output={5} --shift-output={1} --verbose={6} {2} - {3}/{4}.wav".format(
                        wav_original_pipe, shift_output, reverberate_opts, rvb_dir, recording_id, normalize_output, verbose)

            new_recording_id = GetNewId(recording_id, prefix, i)
            corrupted_wav_scp[new_recording_id] = wav_corrupted_pipe

    WriteDictToFile(corrupted_wav_scp, output_dir + "/addNoise.sh")
...
def CreateReverberatedCopy(input_dir,
                           ...
                           isotropic_noise_addition_probability, # Probability of adding isotropic noises
                           pointsource_noise_addition_probability, # Probability of adding point-source noises
                           max_noises_adding, # maximum number of point-source noises that can be added to a recording according to its duration
                           rvb_dir, # reverberation waves direcory
                           normalize_output, # scale so that the signal energy is the same as the original input signal. 
                           verbose # Verbose level
                           ):
    ...
    if not os.path.isfile(input_dir + "/reco2dur"):
        ...
        for value in wav_scp.values():
            ...
            if "sox" in value and "speed" in value:
                read_entire_file="true"
                break
        data_lib.RunKaldiCommand("wav-to-duration --read-entire-file={1} scp,p:{0}/wav.scp ark,t:{0}/reco2dur".format(input_dir, read_entire_file))
    ...
    GenerateReverberatedWavScp(wav_scp, durations, output_dir, room_dict, pointsource_noise_list, iso_noise_dict,
               foreground_snr_lower, foreground_snr_upper, background_snr_lower, background_snr_upper, num_replicas,
               include_original, prefix, speech_rvb_probability, shift_output, isotropic_noise_addition_probability, 
               pointsource_noise_addition_probability, max_noises_adding, rvb_dir, normalize_output, verbose)
...
def Main():
    ...
    CreateReverberatedCopy(input_dir = args.input_dir,
                           ...
                           foreground_snr_lower = args.foreground_snr_lower,
                           foreground_snr_upper = args.foreground_snr_upper,
                           background_snr_lower = args.background_snr_lower,
                           background_snr_upper = args.background_snr_upper,
                           ...
                           max_noises_adding = args.max_noises_adding,
                           rvb_dir = args.rvb_dir,
